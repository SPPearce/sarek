...
workflow FASTQ_ALIGN_BWAMEM_MEM2_DRAGMAP_SENTIEON {
    ...
    bam = bam.mix(FGBIO_COPYUMIFROMREADNAME.out.bam_and_bai.map{ meta, bam, bai -> [ meta, bam ] })

    bai = FGBIO_COPYUMIFROMREADNAME.out.bam_and_bai.map{ meta, bam, bai -> [ meta, bai ] }
